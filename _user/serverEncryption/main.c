...
static ssize_t server_handleWrite(oid_t *oid, const void *data, size_t len, struct Buffer * buffer)
{
	...
	for(; i < len; ++i)
	{
		...
		{
			if (rotValue > (maxChar - currentChar)) {
	...
	for (;;) {
		...
		if (err < 0) {
			if (err == -EINTR) {
				...
				fprintf(stderr, "serverEncryption: msgRecv returned %d (%s)\r\n", err, strerror(err));
				exit(EXIT_FAILURE);
			}
		}
...
int main(void)
{
	...
	if (portCreate(&oid.port) < 0) {
		fprintf(stderr, "serverEncryption: portCreate failed\r\n");
		return EXIT_FAILURE;
	}
	...
	if (create_dev(&oid, "serverEncryption") < 0) {
		fprintf(stderr, "serverEncryption: create_dev failed\r\n");
		return EXIT_FAILURE;
	}
